    ...
    isset($_GET['department']) && isset($_GET['type']))
{
    ...
    if (is_resource($ret) && mysql_num_rows($ret))
    {
        ...
        $query = "SELECT id FROM dtc_team_captain WHERE name = '$fullname' AND teamId = '$teamId'";
        $result = mysql_query($query, $conn);
        if(is_resource($result) && mysql_num_rows($result)==0 ){
            ...
            $sql = "UPDATE dtc_join_user
                    SET fullname='{$fullname}',employeeNumber='{$employeeNumber}',phone='{$phone}',department='{$department}',type='{$type}',teamId='{$teamId}', is_allow='1',is_captain='1'
                    where openid= '{$openid}' ";
